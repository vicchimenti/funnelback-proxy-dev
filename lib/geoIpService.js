 ...
 * Enhanced with better logging, caching, and handling of different IP types.
 * 
 ...
 * Enhanced with better logging and detection of different IP types
 * 
 * @param {string} ip - IP address to look up
 * @param {string} [requestId] - Optional request ID for tracking
 * @returns {Promise<Object>} Location data
 */
async function getLocationData(ip, requestId = null) {
    // Generate request ID if not provided
    const trackingId = requestId || `geo_${Date.now()}_${Math.random().toString(36).substring(2, 10)}`;
