 ...
 * Supports Express, Vercel Edge Functions, and generic request objects.
 *
 * @param {Object} req - Request object (Express, Node, or Fetch-style)
 * @returns {string|null} The best available client IP address, or null if not found.
 */
function extractClientIp(req) {
  const headers = req.headers || {};

  // Header getter that works for both plain objects and Fetch-style Headers
  ...
  // Return early if middleware already resolved this
  const originalIp = getHeader('x-original-client-ip');
  if (originalIp) return originalIp;

  // Prioritized IP sources
  const xRealIp = getHeader('x-real-ip');
  const xForwardedFor = getHeader('x-forwarded-for');
  const firstForwardedIp = xForwardedFor?.split(',')[0]?.trim();
  const xVercelProxiedFor = getHeader('x-vercel-proxied-for');
...
/**
 * Creates a standardized log entry with consistent format
 * 
 * @param {string} level - Log level ('info', 'warn', 'error', 'debug')
 * @param {string} event - Event name/description
 * @param {string} handlerName - Name of the handler creating the log
 * @param {Object} data - Additional data to include
 * @param {boolean} [redactIp=true] - Whether to redact IP addresses
 * @returns {string} Stringified JSON log entry
 */
function createLogEntry(level, event, handlerName, data = {}, redactIp = true) {

  // If we have clientIp in data and redactIp is true, redact it
  if (data.clientIp && redactIp) {
    // Redact IP for privacy in general logs
    const ipParts = data.clientIp.split('.');
    if (ipParts.length === 4) {
      data.clientIp = `${ipParts[0]}.${ipParts[1]}.${ipParts[2]}.**`;
    } else {
      data.clientIp = 'IP_REDACTED';
    }
  }
...
/**
 * Logs comprehensive IP information for debugging purposes
 * 
 * @param {Object} req - Express request object
 * @param {string} handlerName - Handler name
 * @param {string} [requestId] - Request ID for tracking
 */
function logFullIpInfo(req, handlerName, requestId = null) {
